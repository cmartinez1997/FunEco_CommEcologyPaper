...
# fern tryin to do repeat measures
...
raw_traits <- read_csv("data/5Year_TraitTable.csv")
spp_pool <- raw_traits$spp
# unlogging!

# now only keep species that are in the trait table (rare spp removed)

data2020_r <- read_csv("data/CoverSep2020.csv")
data2020 <- data2020_r[, -which(names(data2020_r) %in% spp_pool)]
data2020$plot <- data2020_r$plot ; data2020$severity <- data2020_r$severity

# now we need to figure out some way to add in zeroes for species that are 
# in the pool but absent in this year
...
######
traits <- raw_traits %>%
  # filter(!(spp %in% rare$x)) %>% 
  filter(spp %in% colnames(data2020)) %>%
